            ...
            ScrollView {
                VStack(spacing: Theme.Spacing.lg) {
                    ...
                    ProgressStatsSection(
                        ...
                        weightEntries: filteredWeightEntries,
                        useMetric: useMetric,
                        targetWeightKg: userProfiles.first?.targetWeightKg
                    )
                }
    ...
    var body: some View {
        VStack(alignment: .leading, spacing: Theme.Spacing.sm) {
            if metric == .weight && !weightEntries.isEmpty {
                WeightChart(entries: weightEntries, useMetric: useMetric)
            } else {
                PlaceholderChart()
            }
        }
        ...
        Chart {
            ForEach(entries) { entry in
                LineMark(
                    ...
                    y: .value("Weight", useMetric ? entry.weightKg : UnitConverter.kgToLb(entry.weightKg))
                )
...
struct WeightProgressStats: View {
    ...
    var goalProgressPercent: Double? {
        guard let current = currentWeight,
              ...
              let target = targetWeightKg else {
            return nil
        }

        let total = start - target
        guard total != 0 else { return nil }
        let progress = (start - current) / total
        return min(max(progress, 0), 1)
    }

    var goalDirectionIsLoss: Bool? {
        guard let start = startWeight, let target = targetWeightKg else { return nil }
        return target < start
    }
    ...
    var body: some View {
        VStack(spacing: Theme.Spacing.md) {
            HStack(spacing: Theme.Spacing.md) {
                ProgressStatCard(
                    title: "Current",
                    value: currentWeight.map { String(format: "%.1f", useMetric ? $0 : UnitConverter.kgToLb($0)) } ?? "—",
                ...
                ProgressStatCard(
                    title: "Change",
                    value: change.map { String(format: "%+.1f", useMetric ? $0 : UnitConverter.kgToLb($0)) } ?? "—",
                    unit: useMetric ? "kg" : "lb",
            ...
            if let target = targetWeightKg {
                let targetDisplay = useMetric ? target : UnitConverter.kgToLb(target)
                HStack(spacing: Theme.Spacing.md) {
                    ProgressStatCard(
                        title: "Goal",
                        value: String(format: "%.1f", targetDisplay),
                        unit: useMetric ? "kg" : "lb",
                        color: Theme.Colors.accent
                    )

                    if let progress = goalProgressPercent {
                        ProgressStatCard(
                            title: "Progress",
                            value: String(format: "%.0f%%", progress * 100),
                            unit: goalDirectionIsLoss == true ? "to goal" : "toward",
