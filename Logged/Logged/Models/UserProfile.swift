...
/// Macro calculation plan
enum MacroPlan: String, Codable, CaseIterable {
    case balanced
    ...
    var defaultPercents: (protein: Double, carbs: Double, fat: Double) {
        switch self {
        case .balanced: return (protein: 30, carbs: 40, fat: 30)
        case .highProtein: return (protein: 35, carbs: 35, fat: 30)
        case .lowCarb: return (protein: 35, carbs: 25, fat: 40)
        case .keto: return (protein: 25, carbs: 5, fat: 70)
        case .custom: return (protein: 30, carbs: 40, fat: 30)
        }
    }
}
...
final class UserProfile {
    ...
    init(
        ...
        macroProteinPercent: Double = 30,
        macroCarbsPercent: Double = 40,
        macroFatPercent: Double = 30,
        useManualMacros: Bool = false,
        manualCalories: Int? = nil,
        manualProteinG: Double? = nil,
        manualCarbsG: Double? = nil,
        manualFatG: Double? = nil,
        useMetric: Bool = false,
        dailyWaterGoalMl: Int = Constants.Defaults.dailyWaterGoalMl,
        proteinPerKg: Double = Constants.Defaults.proteinPerKg
    ) {
        self.id = id
        self.email = email
        ...
        self.macroPlan = macroPlan
        self.macroProteinPercent = macroProteinPercent
        self.macroCarbsPercent = macroCarbsPercent
        self.macroFatPercent = macroFatPercent
        self.useManualMacros = useManualMacros
    ...
    var heightDisplay: String {
        ...
        if useMetric {
            ...
            return UnitConverter.heightStringFromCm(height)
        }
    }
    ...
    var weightDisplay: String {
        ...
        if useMetric {
            ...
            return String(format: "%.1f lb", UnitConverter.kgToLb(weight))
        }
    }
}
