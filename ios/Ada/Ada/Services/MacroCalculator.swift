...
final class MacroCalculator {
    ...
    /// Calories per kg of body weight change (~7700 kcal per kg of fat)
    ...
    func calculate(
        ...
        let proteinRatio = proteinPerKg ?? defaultProteinPerKg
        ...
        var proteinG = round(weightKg * proteinRatio * 10) / 10
        ...
        if proteinCalories < minProteinCalories {
            proteinG = round(minProteinCalories / 4 * 10) / 10
        ...
        let carbsG = round(max(remainingCalories / 4, 0) * 10) / 10
